...
function parseArgs() {
  ...
  const p = { mode: 'both', port: 5000, hub: 'http://localhost:5000', name: os.hostname(), interval: 1000, repoDir: path.resolve(__dirname), updateBranch: 'main', updateInterval: 60 };
...
const cfg = parseArgs();
console.log(`\n  NEXUS  —  mod: ${cfg.mode.toUpperCase()}\n  ${cfg.mode !== 'agent' ? `hub: http://localhost:${cfg.port}` : `agent -> ${cfg.hub} (${cfg.name})`}\n`);

if (cfg.mode === 'hub' || cfg.mode === 'both') {
  const { startHub } = require('./hub.js');
  startHub(cfg.port, { repoDir: cfg.repoDir, updateBranch: cfg.updateBranch, updateInterval: cfg.updateInterval });
}
if (cfg.mode === 'agent' || cfg.mode === 'both') {
  setTimeout(() => {
    const { startAgent } = require('./agent.js');
    startAgent(cfg.hub, cfg.name, cfg.interval, { repoDir: cfg.repoDir, updateBranch: cfg.updateBranch });
  }, cfg.mode === 'both' ? 800 : 0);
}
